...
%
%bouin='originalgravity_data.dat';
clc
clear all 
close all
% reads the 2D Bouguer anomaly text file data 
bouguer='2D_BA.txt';
fid=fopen(bouguer);
bouguer=fscanf(fid,'%f',[600 450]);    %open the input gravity file, read it and stores it in variable 'bou'
fclose(fid);
bouguer=bouguer';
surf(bouguer)
title("Unfiltered BA");
bouin='2D_BA.txt';
%bouin='2D_BA.txt'; %path and name of the input gravity file, e.g. c:\gravityinput.dat
%bouin=reshape(bouin,[600,450]);
topoout='topooutput.txt';  %path and name of the output topography file, e.g. c:\topooutput.dat
bouinverted='bouinv.txt';   %path and name of the output gravity file, e.g. c:\bouinv.dat
numrows=450;                  %e. g.  112 data in Y direction
numcolumns=600;           % e. g. 112 data in X direction
longx=600;              %e.g. 666 Km data length in X direction
longy=450;            %e.g. 666 Km data length in Y direction
contrast=0.4;          %density contrast value in g/cm3, e. g. 0.4
criterio=0.02;		%convergence criterion in Km, e. g. 0.02
%Z=(30:5:55);   %mean reference depth in Km, e. g. 30 Km
%rmsval=[];
%for i=1:length(Z)
    %z0=Z(i);
    z0=48;
WH=0.002;                %smaller cut-off frequency (1/wavelength in Km) e. g. 0.01, i.e. 1/100 Km
SH=0.005;                %greater cut-off frequency (1/wavelength in Km) e. g. 0.012, i.e. 1/83.3 Km
...
%It starts the computation of the second term of the series with a maximum of 10 iterations
                                                                  %the function 'prod' computes the factorial of the number inside the brackets  
topo2=-(((frequencytotal.^(1))./(prod(1:2))).*(fft2(topoinverse.^2)));   %%==========(1)======== change the sign 
...
%surf(finaltopoinverse)
 if rms2>=criterio   %it determines if the rms error is greater than the convergence criterion
    topo3=-(((frequencytotal.^(1))./(prod(1:2))).*(fft2(finaltopoinverse.^2)))...                                                                       %%=======(2)========== change the sign
    +((((frequencytotal.^(2))./(prod(1:3))).*(fft2(finaltopoinverse.^3))));   %the third term of the series is computed using the new topography approach  
...
topo3=topo3.*filter; %the filter is applied to the third term of the series
	...
	topo3=constant-topo3;  %the new topography approach is computed
	 ...
	 if rms3>=criterio  %it determines if the convergence criterion has been reached or not
    	topo4=-(((frequencytotal.^(1))/(prod(1:2))).*(fft2(topoinverse3.^2)))...                        %%=======(3)========== change the sign
            +((((frequencytotal.^(2))/(prod(1:3))).*(fft2(topoinverse3.^3))))...
        -((((frequencytotal.^(3))/(prod(1:4))).*(fft2(topoinverse3.^4))));  %the fourth term of the series is computed, and so on...
...
topo4=topo4.*filter;
     ...
     % surf(finaltopoinverse)
      iter=4;
       if rms4>=criterio
         topo5=-(((frequencytotal.^(1))/(prod(1:2))).*(fft2(topoinverse4.^2)))...%%=======(4)========== change the sign 
             +((((frequencytotal.^(2))/(prod(1:3))).*(fft2(topoinverse4.^3))))...
             -((((frequencytotal.^(3))/(prod(1:4))).*(fft2(topoinverse4.^4))))...
             +((((frequencytotal.^(4))/(prod(1:5))).*(fft2(topoinverse4.^5))));
...
topo5=topo5.*filter;
         ...
         %surf(finaltopoinverse+z0)
         iter=5;
         if rms5>=criterio
            topo6=-(((frequencytotal.^(1))/(prod(1:2))).*(fft2(topoinverse5.^2)))... %%======(5)=========== change the sign
                +((((frequencytotal.^(2))/(prod(1:3))).*(fft2(topoinverse5.^3))))...
                -((((frequencytotal.^(3))/(prod(1:4))).*(fft2(topoinverse5.^4))))...
                +((((frequencytotal.^(4))/(prod(1:5))).*(fft2(topoinverse5.^5))))...
                -((((frequencytotal.^(5))/(prod(1:6))).*(fft2(topoinverse5.^6))));
...
topo6=topo6.*filter;
           ...
           % surf(finaltopoinverse)
            iter=6;
            if rms6>=criterio
               topo7=-(((frequencytotal.^(1))/(prod(1:2))).*(fft2(topoinverse6.^2)))...%%=======(6)========== change the sign
                   +((((frequencytotal.^(2))/(prod(1:3))).*(fft2(topoinverse6.^3))))...
                   -((((frequencytotal.^(3))/(prod(1:4))).*(fft2(topoinverse6.^4))))...
                   +((((frequencytotal.^(4))/(prod(1:5))).*(fft2(topoinverse6.^5))))...
                   -((((frequencytotal.^(5))/(prod(1:6))).*(fft2(topoinverse6.^6))))...
                   +((((frequencytotal.^(6))/(prod(1:7))).*(fft2(topoinverse6.^7))));
...
topo7=topo7.*filter;
               ...
               iter=7;
               if rms7>=criterio
                  topo8=-(((frequencytotal.^(1))/(prod(1:2))).*(fft2(topoinverse7.^2)))...%%======(7)=========== change the sign
                      +((((frequencytotal.^(2))/(prod(1:3))).*(fft2(topoinverse7.^3))))...
                      -((((frequencytotal.^(3))/(prod(1:4))).*(fft2(topoinverse7.^4))))...
                      +((((frequencytotal.^(4))/(prod(1:5))).*(fft2(topoinverse7.^5))))...
                      -((((frequencytotal.^(5))/(prod(1:6))).*(fft2(topoinverse7.^6))))...
                      +((((frequencytotal.^(6))/(prod(1:7))).*(fft2(topoinverse7.^7))))...
                      -((((frequencytotal.^(7))/(prod(1:8))).*(fft2(topoinverse7.^8))));
...
topo8=topo8.*filter;
                  ...
                  iter=8;
                  if rms8>=criterio
                     topo9=-(((frequencytotal.^(1))/(prod(1:2))).*(fft2(topoinverse8.^2)))...%%=======(8)========== change the sign
                         +((((frequencytotal.^(2))/(prod(1:3))).*(fft2(topoinverse8.^3))))...
                         -((((frequencytotal.^(3))/(prod(1:4))).*(fft2(topoinverse8.^4))))...
                         +((((frequencytotal.^(4))/(prod(1:5))).*(fft2(topoinverse8.^5))))...
                         -((((frequencytotal.^(5))/(prod(1:6))).*(fft2(topoinverse8.^6))))...
                         +((((frequencytotal.^(6))/(prod(1:7))).*(fft2(topoinverse8.^7))))...
                         -((((frequencytotal.^(7))/(prod(1:8))).*(fft2(topoinverse8.^8))))...
                         +((((frequencytotal.^(8))/(prod(1:9))).*(fft2(topoinverse8.^9))));
...
topo9=topo9.*filter;
                     ...
                     iter=9;
                     if rms9>=criterio
                        topo10=-(((frequencytotal.^(1))/(prod(1:2))).*(fft2(topoinverse9.^2)))...%%=======(9)========== change the sign
                            +((((frequencytotal.^(2))/(prod(1:3))).*(fft2(topoinverse9.^3))))...
                            -((((frequencytotal.^(3))/(prod(1:4))).*(fft2(topoinverse9.^4))))...
                            +((((frequencytotal.^(4))/(prod(1:5))).*(fft2(topoinverse9.^5))))...
                            -((((frequencytotal.^(5))/(prod(1:6))).*(fft2(topoinverse9.^6))))...
                            +((((frequencytotal.^(6))/(prod(1:7))).*(fft2(topoinverse9.^7))))...
                            -((((frequencytotal.^(7))/(prod(1:8))).*(fft2(topoinverse9.^8))))...
                            +((((frequencytotal.^(8))/(prod(1:9))).*(fft2(topoinverse9.^9))))...
                            -((((frequencytotal.^(9))/(prod(1:10))).*(fft2(topoinverse9.^10))));
...
rms
%rmsval(i)=rms;      %it displays the rms error at the end of the iterative process 
...
if sums<=iter
   ...
   sums=3;
   sumtotal=sumtotal-sum2;                                            %%============(10)=========change the sign 
   if sums<=iter                                                      %%================a small mistake: sumas->sums
      ...
      if sums<=iter
         ...
         sums=5;
         sumtotal=sumtotal-sum4;                                      %%==========(11)=========== change the sign
         if sums<=iter
            ...
            if sums<=iter
               ...
               sums=7;
               sumtotal=sumtotal-sum6;                                %%=========(12)=========== change the sign
               if sums<=iter
                  ...
                  if sums<=iter
                     ...
                     sums=9;
                     sumtotal=sumtotal-sum8;                          %%==========(13)========== change the sign
                     if sums<=iter
                        ...
                        if sums<=iter
                           ...
                           sumtotal=sumtotal-sum10;                   %%===========(14)========== change the sign 
                        end
                     end
                  end
               end
